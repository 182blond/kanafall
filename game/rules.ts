import { kanaFor, type Kana, type KanaScript } from '../data/kana.ts'
...
export const matches = (kana: Kana, answer: string) => kana.romaji.includes(normalize(answer))
...
export function chooseKana(
  ...
  const weights = choices.map((k) => 1 + (1 - (mastery[k.id]?.masteryScore ?? 0)) * 2)
...
export function findTarget(enemies: Enemy[], answer: string): Enemy | undefined {
  return enemies
    .filter((e) => e.state === 'falling' && matches(e.kana, answer))
