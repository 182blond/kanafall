...
A cozy Japanese typing game: protect Nilo, a little forest companion, while learning hiragana, katakana and an introductory path of 10 kanji through 25 useful words. Spanish interface, illustrated companion, synthesized audio, no accounts or external services.
...
- **Tu progreso** shows all 46 kana plus the 25-word kanji route, unlock levels and individual mastery. **Ajustes** controls synthesized effects, music, reduced motion and speed.
...
data/kana.ts             46 hiragana + 46 katakana and 25 words built around 10 kanji
...
An answer earns `10 + min(10, floor(combo / 5))` XP. Hiragana, katakana and kanji have separate XP and levels. Each level needs `round(60 × level^1.4)` additional XP; overflow carries forward. A new kana group opens every two levels, with all 46 in that script available at level 19. The five kanji vocabulary units open at levels 1, 3, 5, 7 and 9. Misses and wrong submissions update the requested skill. Mastery affects spawn weights, while immediate repetition is excluded.

`kanafall.save.v1` stores a version 3 schema with separate path XP/levels, shared statistics, skill mastery, unlocked IDs and settings. Version 1 and 2 saves migrate automatically: existing kana progress stays intact and kanji begins at level 1. Level/unlocks are reconstructed from XP when loaded. Saves are written after every answer and settings change. Invalid or newer-version saves are preserved rather than overwritten; storage failures display a notice. Current run position is intentionally not restored. Future cloud persistence can replace `useProgress` without changing the game rules.
...
This is an MVP, designed primarily for desktop/laptop keyboard play. The kanji route is an initial 10-kanji teaching slice rather than a full course. Future work can expand spaced review, audio and sentence context before scaling the content to 100 kanji. No backend, login, multiplayer, commerce, quests or idle mode is implemented.
